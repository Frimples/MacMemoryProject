...
static inline int physical_write_allowed(uint32_t addr) {
  if (!cfg || cfg->physical_write_range_count == 0 || addr >= 0x400000u)
    return 1;

  for (unsigned int i = 0; i < cfg->physical_write_range_count; i++) {
    if (addr >= cfg->physical_write_low[i] && addr < cfg->physical_write_high[i])
      return 1;
  }
  return 0;
}

static inline void filtered_ps_write(uint8_t type, uint32_t addr, uint32_t val) {
  if (physical_write_allowed(addr))
...
void m68k_write_memory_8(unsigned int address, unsigned int value) {
  ...
  filtered_ps_write(OP_TYPE_BYTE, (uint32_t)address, value);
  return;
}
...
void m68k_write_memory_16(unsigned int address, unsigned int value) {
  ...
  if (address & 0x01) {
    filtered_ps_write(OP_TYPE_BYTE, (uint32_t)address, value & 0xFF);
    filtered_ps_write(OP_TYPE_BYTE, (uint32_t)address + 1, (value >> 8) & 0xFF);
    return;
  }

  filtered_ps_write(OP_TYPE_WORD, (uint32_t)address, value);
  return;
}
...
void m68k_write_memory_32(unsigned int address, unsigned int value) {
  ...
  if (address & 0x01) {
    filtered_ps_write(OP_TYPE_BYTE, (uint32_t)address, value & 0xFF);
    filtered_ps_write(OP_TYPE_WORD, (uint32_t)address + 1, htobe16(((value >> 8) & 0xFFFF)));
    filtered_ps_write(OP_TYPE_BYTE, (uint32_t)address + 3, (value >> 24));
    return;
  }

  filtered_ps_write(OP_TYPE_WORD, (uint32_t)address, value >> 16);
  filtered_ps_write(OP_TYPE_WORD, (uint32_t)address + 2, value);
  return;
}
