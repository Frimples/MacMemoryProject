  ...
  while (!feof(in)) {
    ...
    switch (get_config_item_type(cur_cmd)) {
      ...
      case CONFITEM_SCSIIMAGE:
        ...
        break;
      case CONFITEM_PHYSWRITE: {
        unsigned int address = 0, size = 0;
        while (str_pos < (int)strlen(parse_line)) {
          get_next_string(parse_line, cur_cmd, &str_pos, '=');
          if (strcmp(cur_cmd, "address") == 0) {
            get_next_string(parse_line, cur_cmd, &str_pos, ' ');
            address = get_int(cur_cmd);
          } else if (strcmp(cur_cmd, "size") == 0) {
            get_next_string(parse_line, cur_cmd, &str_pos, ' ');
            size = get_int(cur_cmd);
          } else {
            printf("[CFG] Unknown physwrite argument %s on line %d.\n", cur_cmd, cur_line);
            break;
          }
        }
        if (!size || cfg->physical_write_range_count >= MAX_PHYSICAL_WRITE_RANGES ||
            address > 0xFFFFFFFFu - size) {
          printf("[CFG] Invalid physwrite range on line %d.\n", cur_line);
          break;
        }
        unsigned int i = cfg->physical_write_range_count++;
        cfg->physical_write_low[i] = address;
        cfg->physical_write_high[i] = address + size;
        printf("[CFG] Physical writes enabled for %.8X-%.8X.\n",
               address, address + size - 1);
        break;
      }
      case CONFITEM_FORCE24BIT:
        cfg->force24bit = (get_int(parse_line + str_pos) != 0);
        printf("[CFG] Mac SE forced 24-bit addressing %s.\n",
               cfg->force24bit ? "enabled" : "disabled");
        break;
